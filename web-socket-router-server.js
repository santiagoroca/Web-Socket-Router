...
*
*/
var WebSocketRouter = function (connection, ctx) {
  ...
  */
  function Router (connection, ctx) {
    ...
    if (connection.on) {
      connection.on('message', message => dispatch(JSON.parse(message)));
      ...
      connection.onmessage = message => dispatch(JSON.parse(message.data))
